...
namespace EFUtool
{
    ...
    public class FileData
    {
        ...
        internal FileData(string dir, WIN32_FIND_DATA findData) 
        {
            ...
            this.FullPath = Path.Combine(dir, findData.cFileName);
        }

        public FileData(string dir, string name, FileAttributes attributes, long size, DateTime createdUTC, DateTime modifiedUTC, DateTime accessedUTC)
        {
    ...
    public static class FastDirectory
    {
        ...
        private class FileEnumerator : IEnumerator<FileData>
        {
            ...
            private static extern SafeFindHandle FindFirstFileW(string fileName, 
                [In, Out] WIN32_FIND_DATA data);

            [DllImport("kernel32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
            private static extern bool FindNextFileW(SafeFindHandle hndFindFile, 
            ...
            public bool MoveNext()
            {
                ...
                if (m_currentContext.currPhase != 2)
                {
                    if (m_hndFindFile == null)
                    {
                        ...
                        if (searchPath.Length > 256)
                            searchPath = searchPath.StartsWith(@"\\") ? @"\\?\UNC" + searchPath.Substring(1) : @"\\?\" + searchPath;
                        m_hndFindFile = FindFirstFileW(searchPath, m_win_find_data);
                        retval = !m_hndFindFile.IsInvalid;
                    }
                    else
                    {
                        //Otherwise, find the next item.
                        retval = FindNextFileW(m_hndFindFile, m_win_find_data);
                    }
                }
                ...
                if (retval)
                {
                    ...
                    if (m_searchOption == SearchOption.AllDirectories && ((FileAttributes)m_win_find_data.dwFileAttributes & FileAttributes.Directory) == FileAttributes.Directory)
